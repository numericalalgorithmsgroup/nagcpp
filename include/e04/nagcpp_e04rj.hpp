...
namespace nagcpp {
  namespace opt {
    ...
    // modelling suite and defines the block of linear constraints of the problem.
    ...
    //         Optionally, on entry: if idlc = 0, new linear constraints are added to
    //         the problem definition
    //         On exit, if not null on entry: the number of the last linear constraint
    //         added, thus nclin
    //         default value: 0
      ...
      if (!(opt.fail.initial_error_handler(en_data))) {
        if (opt.fail.ierr == 1 && opt.fail.ifmt == 100) {
          ...
          opt.fail.set_errorid(3, error_handler::ErrorCategory::Error,
                               error_handler::ErrorType::GeneralError);
          opt.fail.append_msg(false, "A set of linear constraints has already "
                                     "been defined.");
