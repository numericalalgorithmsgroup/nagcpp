...
namespace nagcpp {
  namespace opt {
    ...
    // Run an interior point solver on a sparse nonlinear programming problem (NLP)
    // initialized by opt::handle_init (e04ra) and defined by other functions from
    // the suite.
    // Note: this function uses optional algorithmic parameters, see also:
    // opt::handle_opt_set (e04zm), opt::handle_opt_get (e04zn).
    // opt::handle_solve_ipopt (e04st), an interior point method optimization
    // solver, based on the IPOPT software package, is a solver for the NAG
    // optimization modelling suite and is suitable for large scale nonlinear
    ...
    //         is to be evaluated
    //       nnzfd: types::f77_integer, scalar
    //         The number of nonzero elements in the sparse gradient vector of the
    //         objective function, as was set in a previous call to
    //         opt::handle_set_nlnobj (e04rg)
    ...
    //       nnzgd: types::f77_integer, scalar
    //         Is the number of nonzero elements in the sparse Jacobian of the
    //         constraint functions, as was set in a previous call to
    //         opt::handle_set_nlnconstr (e04rk)
    ...
    //       nnzh: types::f77_integer, scalar
    //         The number of nonzero elements in the Hessian to be computed
    ...
    //       u: double, array, shape(nnzu)
    //         If nnzu > 0, u holds the values at the current iteration of Lagrange
    //         multipliers (dual variables) for the constraints
    //       rinfo: double, array, shape(32)
    ...
    //     This option sets the value epsilon_tol which is used for optimality and
    //     complementarity tests from KKT conditions See Stopping Criteria
    //   Task: char
    //     Default = "MINIMIZE"
    ...
    //     comm.handle using opt::handle_init (e04ra).
    //   (errorid 4)
...
#define E04STFT_OBJFUN                                              \
  std::function<void(                                               \
    ...
    const utility::array1D<double, data_handling::ArgIntent::IN> &, double &)>
#endif
...
#define E04STFT_OBJGRD                                              \
  std::function<void(                                               \
    ...
    utility::array1D<double, data_handling::ArgIntent::OUT> &)>
#endif
...
#define E04STFT e04stft_
    ...
    public:
      ...
      friend void handle_solve_ipopt(COMM &comm, OBJFUN objfun, OBJGRD objgrd,
                                     CONFUN confun, CONGRD congrd, HESS hess,
                                     MONIT monit, X &&x,
                                     const types::f77_integer nnzu, U &&u,
                                     RINFO &&rinfo, STATS &&stats,
                                     opt::OptionalE04ST &opt);
    };
    ...
    void e04st_objfunh(const E04STFT_OBJFUN &objfun,
      ...
      utility::array1D<double, data_handling::ArgIntent::IN> local_x(x, nvar);
      ...
      try {
        if (static_cast<bool>((*callbacks).address[0])) {
          objfun(callbacks, en_data, local_x, fx);
        } else {
    ...
    struct e04st_objfun_cs {
      static void
        ...
        auto local_x = data_handling::convert_nag_array_to_user<
          const utility::array1D<double, data_handling::ArgIntent::IN>,
          data_handling::ArgIntent::IN, OBJFUN_X>(x);

        objfun(local_x.get(), fx);
      }
    };
    ...
    void e04st_objgrdh(const E04STFT_OBJGRD &objgrd,
      ...
      error_handler::ExceptionPointer *ep =
        static_cast<error_handler::ExceptionPointer *>(en_data.wrapptr1);
      utility::array1D<double, data_handling::ArgIntent::IN> local_x(x, nvar);
      utility::array1D<double, data_handling::ArgIntent::OUT> local_fdx(fdx,
                                                                        nnzfd);
      data_handling::CallbackAddresses *callbacks =
        static_cast<data_handling::CallbackAddresses *>(en_data.wrapptr2);
      ...
      try {
        if (static_cast<bool>((*callbacks).address[1])) {
          objgrd(callbacks, en_data, local_x, nnzfd, local_fdx);
        } else {
    ...
    struct e04st_objgrd_cs {
      static void
        run(const data_handling::CallbackAddresses *callbacks,
            types::engine_data &en_data,
            const utility::array1D<double, data_handling::ArgIntent::IN> &x,
            const types::f77_integer nnzfd,
            utility::array1D<double, data_handling::ArgIntent::OUT> &fdx) {
        OBJGRD objgrd =
        ...
        auto local_x = data_handling::convert_nag_array_to_user<
          const utility::array1D<double, data_handling::ArgIntent::IN>,
          data_handling::ArgIntent::IN, OBJGRD_X>(x);
        auto local_fdx = data_handling::convert_nag_array_to_user<
          utility::array1D<double, data_handling::ArgIntent::OUT>,
          data_handling::ArgIntent::OUT, OBJGRD_FDX>(fdx);

        objgrd(local_x.get(), nnzfd, local_fdx.get());
      }
    };
    ...
    void e04st_confunh(const E04STFT_CONFUN &confun,
      ...
      error_handler::ExceptionPointer *ep =
        static_cast<error_handler::ExceptionPointer *>(en_data.wrapptr1);
      utility::array1D<double, data_handling::ArgIntent::IN> local_x(x, nvar);
      utility::array1D<double, data_handling::ArgIntent::OUT> local_gx(gx,
                                                                       ncnln);
      data_handling::CallbackAddresses *callbacks =
        static_cast<data_handling::CallbackAddresses *>(en_data.wrapptr2);
      ...
      try {
        if (static_cast<bool>((*callbacks).address[2])) {
          confun(callbacks, en_data, local_x, ncnln, local_gx);
        } else {
    ...
    struct e04st_confun_cs {
      static void
        run(const data_handling::CallbackAddresses *callbacks,
            ...
            const utility::array1D<double, data_handling::ArgIntent::IN> &x,
            const types::f77_integer ncnln,
            utility::array1D<double, data_handling::ArgIntent::OUT> &gx) {
        CONFUN confun =
        ...
        auto local_x = data_handling::convert_nag_array_to_user<
          const utility::array1D<double, data_handling::ArgIntent::IN>,
          data_handling::ArgIntent::IN, CONFUN_X>(x);
        auto local_gx = data_handling::convert_nag_array_to_user<
          utility::array1D<double, data_handling::ArgIntent::OUT>,
          data_handling::ArgIntent::OUT, CONFUN_GX>(gx);

        confun(local_x.get(), ncnln, local_gx.get());
      }
    };
    ...
    void e04st_congrdh(const E04STFT_CONGRD &congrd,
      ...
      error_handler::ExceptionPointer *ep =
        static_cast<error_handler::ExceptionPointer *>(en_data.wrapptr1);
      utility::array1D<double, data_handling::ArgIntent::IN> local_x(x, nvar);
      utility::array1D<double, data_handling::ArgIntent::OUT> local_gdx(gdx,
                                                                        nnzgd);
      data_handling::CallbackAddresses *callbacks =
        static_cast<data_handling::CallbackAddresses *>(en_data.wrapptr2);
      ...
      try {
        if (static_cast<bool>((*callbacks).address[3])) {
          congrd(callbacks, en_data, local_x, nnzgd, local_gdx);
        } else {
    ...
    struct e04st_congrd_cs {
      static void
        run(const data_handling::CallbackAddresses *callbacks,
            types::engine_data &en_data,
            const utility::array1D<double, data_handling::ArgIntent::IN> &x,
            const types::f77_integer nnzgd,
            utility::array1D<double, data_handling::ArgIntent::OUT> &gdx) {
        CONGRD congrd =
        ...
        auto local_x = data_handling::convert_nag_array_to_user<
          const utility::array1D<double, data_handling::ArgIntent::IN>,
          data_handling::ArgIntent::IN, CONGRD_X>(x);
        auto local_gdx = data_handling::convert_nag_array_to_user<
          utility::array1D<double, data_handling::ArgIntent::OUT>,
          data_handling::ArgIntent::OUT, CONGRD_GDX>(gdx);

        congrd(local_x.get(), nnzgd, local_gdx.get());
      }
    };
    ...
    void e04st_hessh(const E04STFT_HESS &hess, types::engine_data &en_data,
      ...
      error_handler::ExceptionPointer *ep =
        static_cast<error_handler::ExceptionPointer *>(en_data.wrapptr1);
      utility::array1D<double, data_handling::ArgIntent::IN> local_x(x, nvar);
      utility::array1D<double, data_handling::ArgIntent::IN> local_lamda(lamda,
                                                                         ncnln);
      utility::array1D<double, data_handling::ArgIntent::OUT> local_hx(hx,
                                                                       nnzh);
      data_handling::CallbackAddresses *callbacks =
        static_cast<data_handling::CallbackAddresses *>(en_data.wrapptr2);
      ...
      try {
        if (static_cast<bool>((*callbacks).address[4])) {
          hess(callbacks, en_data, local_x, idf, sigma, local_lamda, nnzh,
               local_hx);
        } else {
    ...
    struct e04st_hess_cs {
      static void
        run(const data_handling::CallbackAddresses *callbacks,
            ...
            const utility::array1D<double, data_handling::ArgIntent::IN> &x,
            const types::f77_integer idf, const double sigma,
            const utility::array1D<double, data_handling::ArgIntent::IN> &lamda,
            const types::f77_integer nnzh,
            utility::array1D<double, data_handling::ArgIntent::OUT> &hx) {
        HESS hess = *((typename std::remove_reference<HESS>::type *)(*callbacks)
                        .address[4]);

        auto local_x = data_handling::convert_nag_array_to_user<
          const utility::array1D<double, data_handling::ArgIntent::IN>,
          data_handling::ArgIntent::IN, HESS_X>(x);
        auto local_lamda = data_handling::convert_nag_array_to_user<
          const utility::array1D<double, data_handling::ArgIntent::IN>,
          data_handling::ArgIntent::IN, HESS_LAMDA>(lamda);
        auto local_hx = data_handling::convert_nag_array_to_user<
          utility::array1D<double, data_handling::ArgIntent::OUT>,
          data_handling::ArgIntent::OUT, HESS_HX>(hx);

        hess(local_x.get(), idf, sigma, local_lamda.get(), nnzh,
             local_hx.get());
      }
    };
    ...
    void e04st_monith(const E04STFT_MONIT &monit, types::engine_data &en_data,
      ...
      error_handler::ExceptionPointer *ep =
        static_cast<error_handler::ExceptionPointer *>(en_data.wrapptr1);
      utility::array1D<double, data_handling::ArgIntent::IN> local_x(x, nvar);
      utility::array1D<double, data_handling::ArgIntent::IN> local_u(u, nnzu);
      utility::array1D<double, data_handling::ArgIntent::IN> local_rinfo(rinfo,
                                                                         32);
      utility::array1D<double, data_handling::ArgIntent::IN> local_stats(stats,
                                                                         32);
      data_handling::CallbackAddresses *callbacks =
        static_cast<data_handling::CallbackAddresses *>(en_data.wrapptr2);
    ...
    struct e04st_monit_cs {
      static void run(
        ...
        types::engine_data &en_data,
        const utility::array1D<double, data_handling::ArgIntent::IN> &x,
        const utility::array1D<double, data_handling::ArgIntent::IN> &u,
        const utility::array1D<double, data_handling::ArgIntent::IN> &rinfo,
        const utility::array1D<double, data_handling::ArgIntent::IN> &stats) {
        MONIT monit =
    ...
    void handle_solve_ipopt(COMM &comm, OBJFUN objfun, OBJGRD objgrd,
                            CONFUN confun, CONGRD congrd, HESS hess,
                            MONIT monit, X &&x, const types::f77_integer nnzu,
                            U &&u, RINFO &&rinfo, STATS &&stats,
                            opt::OptionalE04ST &opt) {
      ...
      using local_objfun_x_t = callback_handling::get_argument_type_t<
        0, callback_handling::argument_type_of_t<OBJFUN>>;
      auto local_objfun = e04st_objfun_cs<local_objfun_x_t, OBJFUN>::run;
      using local_objgrd_x_t = callback_handling::get_argument_type_t<
        0, callback_handling::argument_type_of_t<OBJGRD>>;
      using local_objgrd_fdx_t = callback_handling::get_argument_type_t<
        2, callback_handling::argument_type_of_t<OBJGRD>>;
      auto local_objgrd =
        e04st_objgrd_cs<local_objgrd_x_t, local_objgrd_fdx_t, OBJGRD>::run;
      using local_confun_x_t = callback_handling::get_argument_type_t<
        0, callback_handling::argument_type_of_t<CONFUN>>;
      using local_confun_gx_t = callback_handling::get_argument_type_t<
        2, callback_handling::argument_type_of_t<CONFUN>>;
      auto local_confun =
        e04st_confun_cs<local_confun_x_t, local_confun_gx_t, CONFUN>::run;
      using local_congrd_x_t = callback_handling::get_argument_type_t<
        0, callback_handling::argument_type_of_t<CONGRD>>;
      using local_congrd_gdx_t = callback_handling::get_argument_type_t<
        2, callback_handling::argument_type_of_t<CONGRD>>;
      auto local_congrd =
        e04st_congrd_cs<local_congrd_x_t, local_congrd_gdx_t, CONGRD>::run;
      using local_hess_x_t = callback_handling::get_argument_type_t<
        0, callback_handling::argument_type_of_t<HESS>>;
      using local_hess_lamda_t = callback_handling::get_argument_type_t<
        3, callback_handling::argument_type_of_t<HESS>>;
      using local_hess_hx_t = callback_handling::get_argument_type_t<
        5, callback_handling::argument_type_of_t<HESS>>;
      auto local_hess = e04st_hess_cs<local_hess_x_t, local_hess_lamda_t,
                                      local_hess_hx_t, HESS>::run;
      using local_monit_x_t = callback_handling::get_argument_type_t<
        0, callback_handling::argument_type_of_t<MONIT>>;
      using local_monit_u_t = callback_handling::get_argument_type_t<
        1, callback_handling::argument_type_of_t<MONIT>>;
      using local_monit_rinfo_t = callback_handling::get_argument_type_t<
        2, callback_handling::argument_type_of_t<MONIT>>;
      using local_monit_stats_t = callback_handling::get_argument_type_t<
        3, callback_handling::argument_type_of_t<MONIT>>;
      auto local_monit =
        e04st_monit_cs<local_monit_x_t, local_monit_u_t, local_monit_rinfo_t,
                       local_monit_stats_t, MONIT>::run;
      ...
      data_handling::RawData<double, data_handling::ArgIntent::OUT,
        ...
        local_u(u);
      local_u.resize(u, nnzu);
      data_handling::RawData<double, data_handling::ArgIntent::OUT,
                             typename std::remove_reference<RINFO>::type>
        local_rinfo(rinfo);
      local_rinfo.resize(rinfo, static_cast<types::f77_integer>(32));
      data_handling::RawData<double, data_handling::ArgIntent::OUT,
                             typename std::remove_reference<STATS>::type>
        local_stats(stats);
      local_stats.resize(stats, static_cast<types::f77_integer>(32));
      void *local_iuser = nullptr;
      void *local_ruser = nullptr;

      local_stats.check(opt.fail, "stats", true,
                        static_cast<types::f77_integer>(32));
      if (opt.fail.error_thrown) {
        return;
      }
      ...
      bool u_is_required = (nnzu > 0);
      local_u.check(opt.fail, "u", u_is_required, nnzu);
      if (opt.fail.error_thrown) {
        return;
      }
      ...
      e04stft_(en_data, local_print_rec, utility::print_rech, &comm.handle,
               ...
               local_x.data, nnzu, local_u.data, local_rinfo.data,
      ...
      if (!(opt.fail.initial_error_handler(en_data))) {
        if (opt.fail.ierr == -199 && opt.fail.ifmt == 100) {
          ...
          opt.fail.set_errorid(3, error_handler::ErrorCategory::Error,
                               error_handler::ErrorType::GeneralError);
          opt.fail.append_msg(false, "A different solver from the suite has "
                                     "already been used. Initialize a new");
          opt.fail.append_msg(false, "comm.handle using opt::handle_init.");
          ...
          opt.fail.append_msg(false,
                              "given during initialization of the comm.handle, "
                              "i.e., ",
                              2, ".");
