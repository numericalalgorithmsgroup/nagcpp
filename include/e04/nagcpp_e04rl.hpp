...
namespace nagcpp {
  namespace opt {
    ...
    // suite and defines the structure of the Hessians of the nonlinear objective
    // and constraints, on assumption that they are present in the problem.
    // Alternatively, it may be used to define the Hessian of the Lagrangian.
    ...
    //     Specifies the quantities for which a sparsity structure is provided in
    //     nnzh, irowh and icolh
      ...
      if (!(opt.fail.initial_error_handler(en_data))) {
        if (opt.fail.ierr == 1 && opt.fail.ifmt == 100) {
          ...
          opt.fail.set_errorid(3, error_handler::ErrorCategory::Error,
                               error_handler::ErrorType::GeneralError);
          opt.fail.append_msg(false, "The structure of the Hessian of the "
                                     "Lagrangian has already been defined.");
        } else if (opt.fail.ierr == 3 && opt.fail.ifmt == 302) {
          opt.fail.set_errorid(3, error_handler::ErrorCategory::Error,
                               error_handler::ErrorType::GeneralError);
          opt.fail.append_msg(false, "The structure of the individual Hessians "
                                     "has already been defined,");
          opt.fail.append_msg(false, "the Hessian of the Lagrangian cannot be "
                                     "defined.");
        } else if (opt.fail.ierr == 3 && opt.fail.ifmt == 303) {
          opt.fail.set_errorid(3, error_handler::ErrorCategory::Error,
                               error_handler::ErrorType::GeneralError);
          opt.fail.append_msg(true, "On entry, idf = ", 1, ".");
          opt.fail.append_msg(false, "The structure of the Hessian of "
                                     "nonlinear function linked to the given "
                                     "idf");
          opt.fail.append_msg(false, "has already been defined.");
          ...
          opt.fail.set_errorid(7, error_handler::ErrorCategory::Error,
                               error_handler::ErrorType::GeneralError);
          opt.fail.append_msg(true, "On entry, idf = ", 1, ".");
          opt.fail.append_msg(false, "Constraint: ", 2, " <= idf <= ", 3, ".");
