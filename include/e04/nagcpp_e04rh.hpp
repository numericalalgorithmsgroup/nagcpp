...
namespace nagcpp {
  namespace opt {
    ...
    // modelling suite and defines bounds on the variables of the problem.
...
#define E04RHFT e04rhft_
    ...
    void handle_set_simplebounds(COMM &comm, const BL &bl, const BU &bu,
      ...
      if (!(opt.fail.initial_error_handler(en_data))) {
        ...
        } else if (opt.fail.ierr == 3 && opt.fail.ifmt == 304) {
          opt.fail.set_errorid(3, error_handler::ErrorCategory::Error,
                               error_handler::ErrorType::GeneralError);
          opt.fail.append_msg(false, "Variable bounds have already been "
                                     "defined.");
