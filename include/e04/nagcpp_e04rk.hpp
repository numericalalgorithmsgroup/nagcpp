...
namespace nagcpp {
  namespace opt {
    ...
    // modelling suite and defines the number of nonlinear constraints of the
    // problem as well as the sparsity structure of their first derivatives.
    ...
    //     nonlinear constraints cannot be added.
    //   (errorid 3)
    ...
    void handle_set_nlnconstr(COMM &comm, const BL &bl, const BU &bu,
      ...
      if (!(opt.fail.initial_error_handler(en_data))) {
        if (opt.fail.ierr == 1 && opt.fail.ifmt == 100) {
          ...
          opt.fail.set_errorid(2, error_handler::ErrorCategory::Error,
                               error_handler::ErrorType::GeneralError);
          opt.fail.append_msg(false, "The Hessian of the nonlinear objective "
                                     "has already been defined,");
          opt.fail.append_msg(false, "nonlinear constraints cannot be added.");
        } else if (opt.fail.ierr == 3 && opt.fail.ifmt == 302) {
          opt.fail.set_errorid(3, error_handler::ErrorCategory::Error,
                               error_handler::ErrorType::GeneralError);
          opt.fail.append_msg(false, "A set of nonlinear constraints has "
