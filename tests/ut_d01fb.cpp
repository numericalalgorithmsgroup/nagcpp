...
// C like function
double clike_arr(const utility::array1D<double, data_handling::ArgIntent::IntentIN> &x) {
  return f_base(x);
}
// C++ std::function
std::function<double(const utility::array1D<double, data_handling::ArgIntent::IntentIN> &x)>
  function_arr = [](const utility::array1D<double, data_handling::ArgIntent::IntentIN> &x) {
    return f_base(x);
  };
// functor
struct functor_arr {
  ...
  double operator()(const utility::array1D<double, data_handling::ArgIntent::IntentIN> &x) {
    return f_base(x);
  }
};
// lambda function
auto lambda_arr = [](const utility::array1D<double, data_handling::ArgIntent::IntentIN> &x) {
  return f_base(x);
};
// functor with data
...
struct test_different_calling_types : public TestCase {
  void run() override {
    ...
    }
    {
      ...
      utility::array1D<types::f77_integer, data_handling::ArgIntent::IntentIN> nptvec(
        example::nptvec.data(), example::nptvec.size());
      utility::array1D<double, data_handling::ArgIntent::IntentIN> weight(
        example::weight.data(), example::weight.size());
      utility::array1D<double, data_handling::ArgIntent::IntentIN> abscis(
