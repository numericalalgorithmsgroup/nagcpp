...
struct test_float_to_double : public TestCase {
  void run() override {
    ...
    data_handling::RawData<double, data_handling::ArgIntent::IntentINOUT, std::vector<float>> rd(
      vf);
...
struct test_int_to_double : public TestCase {
  void run() override {
    ...
    data_handling::RawData<double, data_handling::ArgIntent::IntentINOUT, std::vector<int>> rd(
      vf);
    ASSERT_ARRAY_EQUAL(vf.size(), vf.data(), rd.data);
...
struct test_int_to_f77_integer : public TestCase {
  void run() override {
    std::vector<int> vf = ut::get_expected_values<int>(20);
    data_handling::RawData<types::f77_integer, data_handling::ArgIntent::IntentINOUT,
                           std::vector<int>>
      rd(vf);
    ASSERT_ARRAY_EQUAL(vf.size(), vf.data(), rd.data);
